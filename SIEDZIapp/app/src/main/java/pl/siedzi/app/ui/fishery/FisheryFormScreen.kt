...
fun FisheryFormScreen(
    fishery: Fishery?,
    topoClipUriFromMap: String?,
    onSave: (name: String, address: String, lat: Double, lon: Double, stationNote: String, topoClipUri: String?) -> Unit,
    onOpenMapCrop: (lat: Double, lon: Double) -> Unit,
    onBack: () -> Unit
) {
    ...
    var topoClipUri by remember(fishery, topoClipUriFromMap) {
        mutableStateOf(topoClipUriFromMap ?: fishery?.topoClipUri ?: "")
    }
    LaunchedEffect(topoClipUriFromMap) {
        topoClipUriFromMap?.let { topoClipUri = it }
    }
    ...
    Scaffold(
        ...
        Column(
            ...
            OutlinedButton(
                ...
                Text("Otwórz mapę pełnoekranową")
            }
            ...
            Button(
                onClick = {
                    ...
                    val lon = lonText.toDoubleOrNull() ?: 0.0
                    if (name.isNotBlank()) {
                        onSave(name.trim(), address.trim(), lat, lon, stationNote.trim(), topoClipUri.ifBlank { null })
                    }
                },
                modifier = Modifier.fillMaxWidth(),
